...
import { MockUrawaTicketScraper } from './MockUrawaTicketScraper.ts';

Deno.test('UrawaTicketScraper Tests', async (t) => {
  await t.step('should scrape away tickets successfully', async () => {
    const scraper = new MockUrawaTicketScraper();
  ...
  await t.step('should return custom mock data', async () => {
    ...
    const scraper = new MockUrawaTicketScraper(customData);
    const tickets = await scraper.scrapeTickets();
