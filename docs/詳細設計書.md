...
### 3.1 TicketRepositoryインターフェース
```typescript
// src/features/shared/repositories/TicketRepository.ts
export interface TicketRepository {
  findAll(): Promise<Ticket[]>;
  findById(id: string): Promise<Ticket | null>;
  findFutureTickets(): Promise<Ticket[]>;
  save(ticket: Ticket): Promise<void>;
  update(ticket: Ticket): Promise<void>;
  deleteExpired(beforeDate: Date): Promise<void>;
}
```

### 3.2 SupabaseTicketRepository実装
```typescript
// src/features/away-tickets/repositories/SupabaseTicketRepository.ts
...
export class UrlManager {
  ...
  private constructor() {
    this.config = {
      ...
      dynamicUrls: {
        urawaAwayTabUrl: Deno.env.get('URAWA_AWAY_TAB_URL') || 'https://www.jleague-ticket.jp/club/ur/?tab=away',
        fallbackUrls: [
...
export class AwayTabScrapingService {
  ...
  async scrapeTickets(): Promise<{ success: boolean; tickets: ScrapedTicketData[]; errors: string[] }> {
    let browser = null;
    try {
  ...
  private isValidAwayTicket(ticket: ScrapedTicketData): boolean {
    ...
    // アウェイ戦の特徴確認
    const hasAwayIndicator = 
      !ticket.venue.includes('埼玉スタジアム') ||
...
export class LineNotificationService implements NotificationService {
  ...
  async sendNotification(message: NotificationMessage): Promise<{ success: boolean; error?: string }> {
    try {
...
📅 試合日: ${ticket.matchDate.toLocaleDateString('ja-JP')} ${ticket.matchDate.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}
🎫 販売開始: ${ticket.saleStartDate.toLocaleDateString('ja-JP')} ${ticket.saleStartDate.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}
🏟️ 会場: ${ticket.venue}
...
export class ErrorHandler {
  static async handleScrapingError(error: Error, notificationService: NotificationService): Promise<void> {
  ...
  static async handleDatabaseError(error: Error, notificationService: NotificationService): Promise<void> {
...
export class PerformanceOptimizer {
  static async withMemoryMonitoring<T>(operation: () => Promise<T>, label: string): Promise<T> {
    ...
    try {
      ...
      console.log(`[${label}] Memory diff: ${Math.round((memAfter.heapUsed - memBefore.heapUsed) / 1024 / 1024)}MB`);
  ...
  static async withTimeout<T>(operation: () => Promise<T>, timeoutMs: number, label: string): Promise<T> {
...
この詳細設計書に従って実装することで、堅牢で保守性の高いシステムを構築できます。# urawa-support-hub 詳細設計書
