...
**Created**: 2025-08-22\
**Updated**: 2025-11-06 (最新状況: #179修正完了、#176完了 -
Ticketエンティティのoptionalプロパティnull統一)\
...
### ✅ **最近完了・クローズしたイシュー (2025-11-06)**:

- #179 - [BUG] サンフレッチェ広島のチケット販売状況が「発売中」に更新されない ✅ **NEW**
...
| ----- | -------------------------------------------------------------------- | ------------------ | ----------- | ---------------- |
| #184  | collect-ticketがエラー                                               | スクレイピングバグ | 🔴 Critical | 🚨 **OPEN**      |
| #182  | Ticket Entityのupsert改善：既存値保持と差分更新ロジックの実装        | 機能改善           | 🟠 High     | 🚨 **OPEN**      |
...
| ----- | ----------------------------------------------------------------- | -------------- | ---------------- |
| #181  | SaleStatusServiceのリファクタリング：サイト固有フォーマットを分離 | リファクタ     | 🚨 **OPEN**      |
...
## Next Steps Priority (**UPDATED: 2025-11-06**)

### 🎯 **推奨実装順序**

#### 📍 **最優先タスク** (バグ修正)

1. **#184** - collect-ticketがエラー 🔴 **CRITICAL**
   - **内容**: スクレイピング実行時のエラー修正
   - **期待効果**: システムの安定稼働・チケット情報取得継続

#### 📍 **高優先タスク** (機能改善・開発効率)

2. **#182** - Ticket Entityのupsert改善：既存値保持と差分更新ロジックの実装 🟠 **HIGH PRIORITY**
   - **内容**: Ticketエンティティのupsertロジック改善
   - **期待効果**: データ整合性向上・更新処理の最適化

3. **#171** - 開発環境でのCloudTasksモック実装 🟠 **HIGH PRIORITY**
   - **内容**: ローカル開発環境でのCloud Tasksモック機能実装
   - **期待効果**: 開発効率改善・テスト環境統一

#### 📍 **中優先タスク** (運用改善・パフォーマンス)

4. **#181** - SaleStatusServiceのリファクタリング：サイト固有フォーマットを分離 🟡
   - **内容**: ドメイン層からサイト固有フォーマットを分離
   - **期待効果**: アーキテクチャ改善・保守性向上
...
7. **#177** - スクレイパー間でのログ出力戦略の一貫性を確保 🟢
   - **内容**: ログフォーマット統一・出力レベル調整
   - **期待効果**: 保守性向上・デバッグ効率改善
8. **#67** - ヴィッセル神戸対応 🟢
   - **延期理由**: 現在販売中チケットのみで販売前DOMパターンが不明
   - **実装時期**: 2026年1-2月（新規チケット販売開始時）
9. **#84** - Playwright scraper mock化 🟡
10. **#105** - 2026年シーズン制度変更対応 🟢 (2026年2月まで)
